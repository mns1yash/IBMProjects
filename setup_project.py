...
# Create directories and files using echo to generate the files
def create_structure(base_folder, structure):
    for folder, content in structure.items():
        folder_path = os.path.join(base_folder, folder)
        if isinstance(content, dict):
            # Recursively create directories for nested content
            os.makedirs(folder_path, exist_ok=True)
            create_structure(folder_path, content)
        elif isinstance(content, list):
            # Create directories first
            os.makedirs(folder_path, exist_ok=True)
            # Create each file within the directory using 'echo. > filename'
            for file in content:
